...
const Patients = () => {
  ...
  const { data: patients = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("patients").select("id, name, phone, email, birth_date, cpf, notes, medical_history, initial_anamnesis, procedure_performed, created_at, updated_at, user_id").eq("user_id", user!.id).order("name").limit(500);
      return data || [];
    },
    enabled: !!user,
  });
