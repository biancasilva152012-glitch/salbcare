...
const BookingSuccess = () => {
  ...
  const [recording, setRecording] = useState(!!sessionId);

  useEffect(() => {
    if (!sessionId) return;
    supabase.functions
      .invoke("record-consultation-payment", { body: { session_id: sessionId } })
      .then(({ data, error }) => {
        if (error) console.error("Record payment error:", error);
      })
